...
def test_games_list_empty_initially(client):
...
def test_watches_list_empty_initially(client):
...
def test_notifications_list_empty_initially(client):
...
def test_create_game_with_invalid_product_ref(client):
...
def test_create_game_with_invalid_product_id(client):
...
def test_list_watches_with_invalid_game_id(client):
    """Test filtering watches by non-existent game ID returns empty."""
    response = client.get("/api/watches?game_id=999")
    assert response.status_code == 200
    assert response.json() == []
...
def test_delete_game_returns_404_for_missing_game(client):
    """Test that deleting a non-existent game returns 404."""
    response = client.delete("/api/games/999")
    assert response.status_code == 404


def test_delete_watch_returns_404_for_missing_watch(client):
    """Test that deleting a non-existent watch returns 404."""
    response = client.delete("/api/watches/999")
    assert response.status_code == 404


def test_update_watch_returns_404_for_missing_watch(client):
...
def test_test_watch_returns_404_for_missing_watch(client):
    """Test that testing a non-existent watch returns 404."""
    response = client.post("/api/watches/999/test")
    assert response.status_code == 404


def test_create_watch_with_invalid_email(client):
    """Test that watches with invalid emails are rejected."""
    # First we need to create a game to test watch creation
    # This test assumes game creation would work if we had valid store data
    # For now, test the email validation at the API level
    response = client.post(
        "/api/watches",
        json={
            "game_id": 1,
    ...
    # Should fail with 404 (game not found) or 400 (invalid email)
...
def test_search_007_returns_catalog_results(client, monkeypatch):
    ...
    async def fake_search_unified(query, locale, limit):
        assert query == "007"
        return [
            {
                "id": 1,
...
def test_refresh_due_with_no_games(client):
    """Test refresh-due endpoint when no games are tracked."""
    response = client.post("/api/refresh-due")
    assert response.status_code == 200
    data = response.json()
    assert data["due"] == 0
...
def test_multiple_api_calls_dont_interfere(client):
...
def test_refresh_due_endpoint_exists(client):
