...
def test_upsert_catalog_and_list_deals():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        repo.upsert_catalog_entries([
            ...
            _deal("UP0002-PPSA00002_00-GAMETWO", "Game Two", 25, 4499, 5999),
        ])

        items, total = repo.list_deals(min_discount=40)
        assert total == 1
        assert items[0]["name"] == "Game One"
        assert items[0]["discount_percent"] == 50
        assert items[0]["platforms"] == ["PS5"]

        suggestions = repo.suggest_names("Game", limit=5)
        assert len(suggestions) == 2

        tracked = repo.mark_tracked(items[0]["id"])
