...
print(settings.database_path)   # default: data/ps_price.sqlite3
...
|----------|---------|---------|
| `PS_PRICE_DATABASE_PATH` | `data/ps_price.sqlite3` | Where SQLite file lives |
...
The deals page HTML only shows ~12 games. The real catalog uses Sony's public GraphQL API:

```python
from backend.app.ps_graphql import fetch_category_products, ALL_DEALS_CATEGORY_ID

results = await fetch_category_products(
    client,
    category_id=ALL_DEALS_CATEGORY_ID,  # ~4400 deals for en-us
    locale="en-us",
...
This paginates 100 products per request until all deals are fetched.
...
# Sync all deals into local catalog
result = await service.sync_deals()
# → {"synced": 4405, "fetched": 4405, "locale": "en-us"}

# Unified search: live store + local catalog
...
1. Query PlayStation Store live (HTML search)
2. Query local SQLite catalog in parallel
3. Store results first, then catalog-only matches
4. If the store request fails, catalog results still return
...
4. Kick off initial deals sync in the background (non-blocking)
...
| GET | `/api/search?q=` | Unified catalog + store search |
...
- Project API reference: `docs/API.md`
