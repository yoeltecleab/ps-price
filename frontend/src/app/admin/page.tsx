...
const DATA_TABS: Tab[] = ["users", "watches", "emails", "games", "library", "security"];
...
function dataPanelClass(loading: boolean) {
  return ["holo-panel rounded-[var(--radius-xl)] overflow-x-auto transition-opacity", loading ? "opacity-60" : ""].join(" ");
}
...
export default function AdminDashboardPage() {
  ...
  const fetchUsers = useCallback(
    async (offset = userOffset) => {
      const params = new URLSearchParams({ limit: "50", offset: String(offset) });
      if (userQuery.trim()) params.set("q", userQuery.trim());
      return api<Paginated<AdminUserRow>>(`/api/admin/users?${params}`);
    },
    [userQuery, userOffset],
  );

  const fetchWatches = useCallback(
    async (offset = watchOffset, enabledOnly = watchEnabledOnly) => {
      const params = new URLSearchParams({ limit: "50", offset: String(offset) });
      if (watchQuery.trim()) params.set("q", watchQuery.trim());
      if (enabledOnly) params.set("enabled_only", "true");
      return api<Paginated<AdminWatchRow>>(`/api/admin/watches?${params}`);
    },
    [watchQuery, watchEnabledOnly, watchOffset],
  );

  const fetchEmails = useCallback(
    async (offset = emailOffset) => {
      const params = new URLSearchParams({ limit: "100", offset: String(offset) });
      if (emailStatus) params.set("status", emailStatus);
      return api<Paginated<AdminNotificationRow>>(`/api/admin/notifications?${params}`);
    },
    [emailStatus, emailOffset],
  );

  const fetchGames = useCallback(
    async (offset = gameOffset, onSaleOnly = gameOnSaleOnly) => {
      const params = new URLSearchParams({ limit: "50", offset: String(offset) });
      if (gameQuery.trim()) params.set("q", gameQuery.trim());
      if (onSaleOnly) params.set("on_sale_only", "true");
      return api<Paginated<AdminGameRow>>(`/api/admin/games?${params}`);
    },
    [gameQuery, gameOnSaleOnly, gameOffset],
  );

  const fetchLibrary = useCallback(
    async (offset = libraryOffset) => {
      const params = new URLSearchParams({ limit: "50", offset: String(offset) });
      if (libraryQuery.trim()) params.set("q", libraryQuery.trim());
      return api<Paginated<AdminLibraryRow>>(`/api/admin/library?${params}`);
    },
    [libraryQuery, libraryOffset],
  );

  const fetchSessions = useCallback(
    async (offset = sessionOffset) => {
      const params = new URLSearchParams({ limit: "50", offset: String(offset) });
      if (sessionQuery.trim()) params.set("q", sessionQuery.trim());
      return api<Paginated<AdminSessionRow>>(`/api/admin/sessions?${params}`);
    },
    [sessionQuery, sessionOffset],
  );

  const fetchPasskeys = useCallback(
    async (offset = passkeyOffset) => {
      const params = new URLSearchParams({ limit: "50", offset: String(offset) });
      if (passkeyQuery.trim()) params.set("q", passkeyQuery.trim());
      return api<Paginated<AdminPasskeyRow>>(`/api/admin/passkeys?${params}`);
    },
    [passkeyQuery, passkeyOffset],
  );

  const fetchAltEmails = useCallback(
    async (offset = altEmailOffset) => {
      const params = new URLSearchParams({ limit: "50", offset: String(offset) });
      if (altEmailQuery.trim()) params.set("q", altEmailQuery.trim());
      return api<Paginated<AdminNotificationEmailRow>>(`/api/admin/notification-emails?${params}`);
    },
    [altEmailQuery, altEmailOffset],
  );

  const loadTabData = useCallback(async () => {
    if (!DATA_TABS.includes(tab)) return;
    setTabLoading(true);
    setTabError(null);
    try {
      if (tab === "users") setUsers(await fetchUsers());
      if (tab === "watches") setWatches(await fetchWatches());
      if (tab === "emails") setEmails(await fetchEmails());
      if (tab === "games") setGames(await fetchGames());
      if (tab === "library") setLibrary(await fetchLibrary());
      if (tab === "security") {
        if (securitySection === "sessions") setSessions(await fetchSessions());
        if (securitySection === "passkeys") setPasskeys(await fetchPasskeys());
        if (securitySection === "addresses") setAltEmails(await fetchAltEmails());
      }
    } catch (e) {
  ...
  const refreshAll = useCallback(async () => {
    setError(null);
    await Promise.all([loadOverview(), loadTabData()]);
  }, [loadOverview, loadTabData]);
  ...
  useEffect(() => {
    ...
    if (!user.email_verified) {
      router.replace("/auth/verify?next=/admin");
      return;
    }
    void loadOverview();
  }, [authLoading, user, router, loadOverview]);
  ...
  useEffect(() => {
    if (authLoading || !user?.is_admin || !user.email_verified) return;
    void loadTabData();
  }, [
    authLoading,
    user,
  ...
  async function runSearch<T>(
    ...
    setTabLoading(true);
    try {
      apply(await fetchPage(0));
    } catch (e) {
      setTabError(e instanceof Error ? e.message : "Search failed");
    } finally {
      setTabLoading(false);
    }
  }
  ...
  if (authLoading || !user?.is_admin || !user.email_verified) {
    return (
    ...
    <div className="space-y-8 pb-16">
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          ...
          <p className="mt-2 text-sm text-muted max-w-xl">
            Users, catalog, security, price alerts, and system operations.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="secondary" onClick={() => refreshAll()} loading={overviewLoading || tabLoading}>
            <ArrowsClockwise className="size-4" />
            Refresh
          </Button>
          <Button loading={actionLoading} onClick={() => runAction(() => api("/api/admin/sync?background=false", { method: "POST" }), "Catalog sync started.")}>
            Force catalog sync
          </Button>
        </div>
      </header>
      ...
      {tab === "overview" ? (
        overviewLoading && !overview ? (
          ...
          <p className="text-sm text-muted">Overview unavailable. Try refresh.</p>
        )
      ) : null}

      {tab === "activity" ? (
        overviewLoading && !insights ? (
          <TabLoader />
        ) : insights ? (
          ...
          <p className="text-sm text-muted">Activity data unavailable. Try refresh.</p>
        )
      ) : null}
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[200px] max-w-md">
              <Input
                ...
                onKeyDown={(e) => e.key === "Enter" && void runSearch(() => setUserOffset(0), fetchUsers, setUsers)}
                placeholder="Email or display name"
              />
            </div>
            <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setUserOffset(0), fetchUsers, setUsers)}>
              Search
            </Button>
          </div>
          <div className={dataPanelClass(tabLoading)}>
            {tabLoading && !users ? <TabLoader /> : (
              <table className="w-full text-sm min-w-[900px]">
                ...
                <tbody>
                  ...
                  {!tabLoading && users && !users.items.length ? <EmptyRow colSpan={5} message="No users match your search." /> : null}
                </tbody>
              </table>
            )}
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[200px] max-w-md">
              <Input
                ...
                onKeyDown={(e) => e.key === "Enter" && void runSearch(() => setGameOffset(0), fetchGames, setGames)}
            ...
            <label className="flex items-center gap-2 font-data text-xs text-muted pb-2">
              <input
                type="checkbox"
                checked={gameOnSaleOnly}
                onChange={(e) => {
                  const onSaleOnly = e.target.checked;
                  setGameOnSaleOnly(onSaleOnly);
                  void runSearch(() => setGameOffset(0), (offset) => fetchGames(offset, onSaleOnly), setGames);
                }}
              />
              On sale only
            </label>
            <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setGameOffset(0), fetchGames, setGames)}>
              Search
            </Button>
          </div>
          <div className={dataPanelClass(tabLoading)}>
            {tabLoading && !games ? <TabLoader /> : (
              <table className="w-full text-sm min-w-[800px]">
                ...
                <tbody>
                  ...
                  {!tabLoading && games && !games.items.length ? <EmptyRow colSpan={4} message="No games match your filters." /> : null}
                </tbody>
              </table>
            )}
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[200px] max-w-md">
              <Input
                ...
                onKeyDown={(e) => e.key === "Enter" && void runSearch(() => setLibraryOffset(0), fetchLibrary, setLibrary)}
                placeholder="Game or user email"
              />
            </div>
            <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setLibraryOffset(0), fetchLibrary, setLibrary)}>
              Search
            </Button>
          </div>
          <div className={dataPanelClass(tabLoading)}>
            {tabLoading && !library ? <TabLoader /> : (
              <table className="w-full text-sm min-w-[700px]">
                ...
                <tbody>
                  ...
                  {!tabLoading && library && !library.items.length ? <EmptyRow colSpan={4} message="No library entries found." /> : null}
                </tbody>
              </table>
            )}
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[200px] max-w-md">
              <Input
                ...
                onKeyDown={(e) => e.key === "Enter" && void runSearch(() => setWatchOffset(0), fetchWatches, setWatches)}
                placeholder="Game, email, user"
              />
            </div>
            <label className="flex items-center gap-2 font-data text-xs text-muted pb-2">
              <input
                type="checkbox"
                checked={watchEnabledOnly}
                onChange={(e) => {
                  const enabledOnly = e.target.checked;
                  setWatchEnabledOnly(enabledOnly);
                  void runSearch(() => setWatchOffset(0), (offset) => fetchWatches(offset, enabledOnly), setWatches);
                }}
              />
              Enabled only
            </label>
            <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setWatchOffset(0), fetchWatches, setWatches)}>
              Search
            </Button>
          </div>
          <div className={dataPanelClass(tabLoading)}>
            {tabLoading && !watches ? <TabLoader /> : (
              <table className="w-full text-sm min-w-[900px]">
                <thead>
                  <tr className="border-b border-border/60 font-data text-[11px] uppercase tracking-widest text-muted">
                    <th className="text-left px-4 py-3">Game</th>
                ...
                <tbody>
                  {(watches?.items ?? []).map((row) => (
                    <tr key={row.id} className="border-b border-border/30 hover:bg-surface/40">
                      <td className="px-4 py-3 font-medium text-ink max-w-[200px] truncate">
                        <Link href={`/games/${row.game_id}`} className="hover:text-accent">{row.game_name}</Link>
                      </td>
                      <td className="px-4 py-3 text-muted">{row.user_email ?? "—"}</td>
                      <td className="px-4 py-3 text-muted">{row.email}</td>
                      <td className="px-4 py-3">
                        {row.enabled ? <Badge variant="success">On</Badge> : <Badge variant="warning">Off</Badge>}
                      </td>
                      <td className="px-4 py-3 text-right">{row.current_price_formatted ?? "—"}</td>
                      <td className="px-4 py-3 text-right">
                  ...
                  {!tabLoading && watches && !watches.items.length ? <EmptyRow colSpan={6} message="No watches match your filters." /> : null}
                </tbody>
              </table>
            )}
      ...
      {tab === "emails" ? (
        <section className="space-y-4">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setEmailOffset(0), fetchEmails, setEmails)}>
              Apply
            </Button>
                ...
                <tbody>
                  {(emails?.items ?? []).map((row) => (
                    <Fragment key={row.id}>
                      ...
                      {expandedEmailId === row.id ? (
                        <tr className="bg-surface/30">
                          <td colSpan={5} className="px-4 py-3 text-xs text-muted space-y-2">
                            {row.game_name ? <p><span className="font-medium text-ink">Game:</span> {row.game_name}</p> : null}
                            {row.reason ? <p><span className="font-medium text-ink">Reason:</span> {row.reason}</p> : null}
                  ...
                  {!tabLoading && emails && !emails.items.length ? <EmptyRow colSpan={5} message="No email log entries match your filter." /> : null}
                </tbody>
              </table>
            )}
        ...
        <section className="space-y-4">
            ...
            <>
              <div className="flex flex-wrap gap-3 items-end max-w-md">
                <Input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && void runSearch(() => setSessionOffset(0), fetchSessions, setSessions)}
                  placeholder="User email"
                />
                <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setSessionOffset(0), fetchSessions, setSessions)}>
                  Search
                </Button>
              </div>
              <div className={dataPanelClass(tabLoading)}>
                {tabLoading && !sessions ? <TabLoader /> : (
                  <table className="w-full text-sm min-w-[800px]">
                    ...
                    <tbody>
                      ...
                      {!tabLoading && sessions && !sessions.items.length ? <EmptyRow colSpan={4} message="No active sessions found." /> : null}
                    </tbody>
                  </table>
                )}
            ...
            <>
              <div className="flex flex-wrap gap-3 items-end max-w-md">
                <Input
                  ...
                  value={passkeyQuery}
                  onChange={(e) => setPasskeyQuery(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && void runSearch(() => setPasskeyOffset(0), fetchPasskeys, setPasskeys)}
                  placeholder="User email or key name"
                />
                <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setPasskeyOffset(0), fetchPasskeys, setPasskeys)}>
                  Search
                </Button>
              </div>
              <div className={dataPanelClass(tabLoading)}>
                {tabLoading && !passkeys ? <TabLoader /> : (
                  <table className="w-full text-sm min-w-[700px]">
                    ...
                    <tbody>
                      ...
                      {!tabLoading && passkeys && !passkeys.items.length ? <EmptyRow colSpan={4} message="No passkeys found." /> : null}
                    </tbody>
                  </table>
                )}
          ...
          {securitySection === "addresses" ? (
            <>
              <div className="flex flex-wrap gap-3 items-end max-w-md">
                <Input
                  label="Search notification emails"
                  value={altEmailQuery}
                  onChange={(e) => setAltEmailQuery(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && void runSearch(() => setAltEmailOffset(0), fetchAltEmails, setAltEmails)}
                  placeholder="Email or owner"
                />
                <Button variant="secondary" loading={tabLoading} onClick={() => void runSearch(() => setAltEmailOffset(0), fetchAltEmails, setAltEmails)}>
                  Search
                </Button>
              </div>
              <div className={dataPanelClass(tabLoading)}>
                {tabLoading && !altEmails ? <TabLoader /> : (
                  <table className="w-full text-sm min-w-[700px]">
                    ...
                    <tbody>
                      ...
                      {!tabLoading && altEmails && !altEmails.items.length ? <EmptyRow colSpan={3} message="No notification addresses found." /> : null}
                    </tbody>
                  </table>
                )}
        ...
        overviewLoading && !overview ? (
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="holo-panel-strong rounded-[var(--radius-xl)] p-6 space-y-4">
              ...
              <p className="text-sm text-muted">Last sync: {overview.sync.last_sync ? formatDateTime(overview.sync.last_sync) : "Never"}</p>
              {overview.sync.syncing ? (
                <p className="text-xs text-accent font-data">Sync in progress…</p>
              ) : overview.sync.can_refresh === false ? (
                <p className="text-xs text-muted font-data">
                  Cooldown active — retry in {overview.sync.retry_after_seconds ?? 0}s
                </p>
              ) : null}
          ...
          <p className="text-sm text-muted">Catalog data unavailable. Try refresh.</p>
        )
      ) : null}
      ...
      {tab === "system" ? (
        overviewLoading && !overview ? (
              ...
              <Button
                ...
                onClick={() =>
                  api<{ catalog_total?: number; syncing?: boolean }>("/api/sync-status").then(
                    ...
                    (e) => setError(e instanceof Error ? e.message : "Health check failed"),
                  )
                }
              >
                Ping sync status
          ...
          <p className="text-sm text-muted">System data unavailable. Try refresh.</p>
        )
      ) : null}
    </div>
  );
